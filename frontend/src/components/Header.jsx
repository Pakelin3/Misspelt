...
function Header({ toggleSidebar }) {
    ...
    return (
        ...
        'bg-[var(--color-dark-bg-secondary)]'} border-b border-[#333] sticky top-0 z-20`}>
            ...
            <div className="flex items-center gap-4">
                <button className={`p-2 rounded-full flex items-center gap-1 text-[var(--color-bg-secondary)] font-semibold hover:bg-gray-700 ${theme === 'light' ? 
                    'hover:bg-gray-700' : 'hover:bg-[var(--color-dark-bg-tertiary)]'}`}>
                    PRO <Gem className="w-4 h-4 text-[var(--color-bg-secondary)]" />
                </button>
                <UserCircle2 className={`w-8 h-8 ${theme === 'light' ? 'text-gray-400' : 'text-[var(--color-dark-text-secondary)]'}`} />
            </div>
        </header>
    );
}
